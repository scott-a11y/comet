...
export const env = createEnv({
  server: {
    ...
    // Secret key must start with "sk_" (e.g., sk_test_... or sk_live_...)
    CLERK_SECRET_KEY: z.string().min(1).refine(
      (key) => key.startsWith('sk_'),
      { message: 'CLERK_SECRET_KEY must start with "sk_" (secret key). You may have accidentally used a publishable key.' }
    ),
    SENTRY_DSN: z.string().url().optional(),
  },
  client: {
    // Publishable key must start with "pk_" (e.g., pk_test_... or pk_live_...)
    NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY: z.string().min(1).refine(
      (key) => key.startsWith('pk_'),
      { message: 'NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY must start with "pk_" (publishable key). You may have accidentally used a secret key.' }
    ),
